...
export class HttpClient {
    ...
    async sendGossipMessage(server: GossipServer, message: GossipMessage): Promise<GossipMessage | null> {
        try {
            ...
            const result = await response.json() as GossipMessage;
            return result;
        } catch (error) {
    ...
    async downloadMasterList(url: string): Promise<ServerList | null> {
        try {
            ...
            const result = await response.json() as ServerList;
            return result;
        } catch (error) {
    ...
    async uploadMasterList(updateUrl: string, auth: string, serverList: ServerList): Promise<boolean> {
        try {
